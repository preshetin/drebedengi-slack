...
export async function moveMessage(
  ...
  return {
    text: `:left_right_arrow: Перемещение ${sumFormatted} ${currency} из места хранения ${fromPlace} в место хранения ${place}, ввел(а) <@${user}>`,
    blocks: [
      {
        type: "section",
        text: {
          type: "mrkdwn",
          text: `:left_right_arrow: Перемещение ${sumFormatted} ${currency} из места хранения ${fromPlace} в место хранения ${place}, ввел(а) <@${user}> ${detailsText}`,
        },
      },
    ],
  };
